...
class MVFv3Parser:
    ...
    def parse_mvf_package(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            if file_path.lower().endswith((".zip", ".mvf")):
                ...
                raise ValueError(
                    f"Unsupported file format. MVF v3 requires ZIP package: {file_path}"
                )
    ...
    def _parse_mvf_zip(self, zip_path: str) -> List[Dict[str, Any]]:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_file:
            ...
            kinds_files = [
                f for f in file_names if f.startswith("kinds/") and f.endswith(".json")
            ]
            ...
            geometry_files = [
                f
                for f in file_names
                if f.startswith("geometry/") and f.endswith(".geojson")
            ]
            ...
            if locations_file:
                with zip_file.open(locations_file) as f:
                    ...
                    if isinstance(locations_data, list):
                        self.locations = {
                            "features": [{"properties": loc} for loc in locations_data]
                        }
                    else:
            ...
            for extension_file in file_names:
                if (
                    extension_file.endswith(".geojson")
                    and extension_file
                    not in ["manifest.geojson", "floors.geojson", "locations.geojson"]
                    and not extension_file.startswith("geometry/")
                ):
                    layers_data.extend(
                        self._process_extension_file(zip_file, extension_file)
                    )
    ...
    def _extract_venue_info(self):
        ...
        if (
            self.manifest
            and "features" in self.manifest
            and len(self.manifest["features"]) > 0
        ):
    ...
    def _process_geometry(
        self, floor_id: str, geometry_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for geom_feature in geometry_data["features"]:
            ...
            if (
                "door" in kind_lower
                or "entrance" in kind_lower
                or "entry" in kind_lower
                or "exit" in kind_lower
            ):
                ...
                if geom_type == QgsWkbTypes.PolygonGeometry:
                    ...
                    is_known_connection = (
                        "stair" in kind_lower
                        or "elevator" in kind_lower
                        or "escalator" in kind_lower
                        or "lift" in kind_lower
                        or "stairs" in kind_lower
                        or "elevators" in kind_lower
                    )
                    ...
                    if is_known_connection or is_unknown_point:
                        ...
                        if len(points) <= 3:
                            # DEBUG: print(f"  -> Added to CONNECTIONS: {geom_id} (kind: {geom_kind}) - {'known connection' if is_known_connection else 'unknown/poi point'}")
                            pass
                    else:
    ...
    def _process_locations(self) -> List[Dict[str, Any]]:
        ...
        if "features" in self.locations:
            location_list = [
                f.get("properties", {}) for f in self.locations["features"]
            ]
    ...
    def _configure_line_layer_styling(self, layer: QgsVectorLayer):
        ...
        try:
            ...
            try:
                ...
                width_expr = QgsProperty.fromExpression(
                    "scale_linear(@map_scale, 1000, 100000, 1.1, 0.3)"
                )
                first_layer.setDataDefinedProperty(
                    QgsSymbolLayer.PropertyStrokeWidth, width_expr
                )
            except Exception:
                pass
    ...
    def _find_geometry_by_id(
        self, geometry_id: str, floor_id: str
    ...
    def _convert_geojson_geometry(
        self, geojson_geom: Dict[str, Any]
    ) -> Optional[QgsGeometry]:
            ...
            if geom_type == "Point":
                if len(coordinates) >= 2:
                    return QgsGeometry.fromPointXY(
                        QgsPointXY(coordinates[0], coordinates[1])
                    )
    ...
    def _get_location_fields(self) -> QgsFields:
        ...
        fields.append(QgsField("geometry_id", QMetaType.QString))
        fields.append(
            QgsField("anchor_count", QMetaType.QString)
        )  # Changed to String to avoid type issues
        return fields
