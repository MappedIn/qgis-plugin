...
from qgis.PyQt.QtWidgets import QFileDialog, QMessageBox, QProgressDialog, QComboBox
...
class FetchVenuesThread(QThread):
    ...
    def run(self):
        try:
            ...
            # First authenticate
            auth_success, auth_error = client.authenticate(self.api_key, self.api_secret)
            if not auth_success:
            ...
            if venues_success:
                self.finished.emit(True, "Venues fetched successfully", venues_list or [])
            else:
...
class MappedInMVFImporterDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def validate_inputs(self):
        ...
        if self.get_import_mode() == 'file':
            if not self.selected_file:
                QMessageBox.warning(self, "Invalid Input", "Please select an MVF package file.")
                return False
        else:
            ...
            if not creds['api_secret']:
                QMessageBox.warning(self, "Invalid Input", "Please enter your API secret.")
                return False
            if not creds['venue_id']:
                QMessageBox.warning(self, "No Venue Selected", 
    ...
    def fetch_venues(self):
        ...
        if not api_key or not api_secret:
            QMessageBox.warning(self, "Missing Credentials", "Please enter both API key and secret before fetching venues.")
            return
        ...
        self.fetch_venues_thread = FetchVenuesThread(api_key, api_secret, self._api_client)
    ...
    def on_venues_fetched(self, success, message, venues_list):
        ...
        if success:
            ...
            if venues_list:
                QMessageBox.information(self, "Venues Loaded", f"Successfully loaded {len(venues_list)} venue(s).")
            else:
                QMessageBox.information(self, "No Venues", "No venues found for your account. You can still enter a venue ID manually.")
        else:
            QMessageBox.warning(self, "Fetch Failed", f"Failed to fetch venues:\n\n{message}")
    ...
    def populate_venues_combo(self, venues_list):
        ...
        for venue in venues_list:
            ...
            venue_name = venue.get('name', venue.get('title', venue.get('displayName', 'Unknown Venue')))
    ...
    def fetch_from_api(self):
        ...
        self.progress_dialog = QProgressDialog("Downloading MVF package...", "Cancel", 0, 100, self)
    ...
    def on_api_download_finished(self, success, message, file_path):
        ...
        if success:
            ...
            QMessageBox.critical(self, "API Download Failed", f"Failed to download MVF package:\n\n{message}")
